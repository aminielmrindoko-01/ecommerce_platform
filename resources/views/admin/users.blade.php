...
<p style="color:var(--color-ink-muted);">Legacy marketplace role plus RBAC role assignment. Super Admin assignment is restricted.</p>
<div class="panel" style="margin-top:1rem;overflow:auto;">
<table style="width:100%;border-collapse:collapse;min-width:720px;">
<thead><tr style="text-align:left;border-bottom:1px solid var(--color-border);"><th style="padding:.75rem;">Name</th><th>Email</th><th>Legacy</th><th>RBAC</th><th>Update</th></tr></thead>
<tbody>
@foreach($users as $user)
<tr style="border-bottom:1px solid var(--color-border);">
<td style="padding:.75rem;">{{ $user->name }}@if(! $user->is_active) <span class="chip">inactive</span>@endif</td>
<td>{{ $user->email }}</td>
<td>{{ $user->role }}</td>
...
@foreach($assignableRoles as $rbac)
    ...
    @endphp
    @unless($blocked)
        <option value="{{ $rbac->name }}" @selected($user->roles->contains('name', $rbac->name))>{{ $rbac->display_name }}</option>
    @endunless
@endforeach
</select>
