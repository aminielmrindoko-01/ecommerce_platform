...
<div class="panel" style="text-align:center;padding:2.5rem 1.5rem;max-width:720px;margin:0 auto;">
    ...
    <p style="color:var(--color-ink-muted);margin:0 0 1.25rem;">
        Order <strong>{{ $order->order_number ?? '#'.$order->id }}</strong> is saved.
        Payment status remains <strong>{{ ucfirst($order->payment_status ?? 'pending') }}</strong> until a genuine verified payment is recorded.
    </p>
