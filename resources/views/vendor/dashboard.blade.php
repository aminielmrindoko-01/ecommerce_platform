...
<div class="section-head">
    <div>
        ...
        <p>Your store performance and recent activity</p>
    </div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1rem;" class="section">
    ...
    <div class="admin-stat"><span>Completed</span><strong>{{ $completedOrders }}</strong></div>
    ...
    <div class="panel">
        ...
        @forelse($recentOrders as $order)
            <div style="padding:.65rem 0;border-bottom:1px solid var(--color-border);">
                ...
                <div style="color:var(--color-ink-muted);font-size:.9rem;">{{ ucfirst($order->status) }} · {{ $order->items->count() }} item(s)</div>
            </div>
        @empty
